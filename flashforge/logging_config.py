"""Centralized logging configuration for FlashForge MCP server.
...
def _sanitize_text_for_logging(
    text: str, max_len: int = Config.LOG_MAX_TEXT_LEN
) -> str:
...
def configure_logging(
    ...
    json_console: bool = Config.LOG_JSON_CONSOLE,
    ...
    Args:
        log_level: Minimum logging level for the application (DEBUG, INFO, etc.).
            Defaults to Config.LOG_LEVEL if not specified.
        log_file: Path to log file (relative or absolute).
            Defaults to Config.LOG_FILE if not specified.
        json_console: Whether to output JSON to console (useful for production).
            Defaults to Config.LOG_JSON_CONSOLE if not specified.
        max_bytes: Maximum size per log file before rotation in bytes.
            Defaults to Config.LOG_MAX_BYTES if not specified.
        backup_count: Number of backup files to keep.
            Defaults to Config.LOG_BACKUP_COUNT if not specified.
    """
    ...
    app_logger = logging.getLogger("flashforge")
