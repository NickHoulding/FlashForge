from exceptions import (
    InvalidCredentialsException, MissingRequiredFieldException, 
    UserAlreadyExistsException, DatabaseTransactionException
)
...
async def register(user_data: UserCreate, db: AsyncSession = Depends(get_db)) -> Token:
    ...
    Raises:
        MissingRequiredFieldException: If required fields are missing (400 Bad Request).
        UserAlreadyExistsException: If the username already exists (409 Conflict).
        DatabaseTransactionException: If there is a server error during registration (500 Internal Server Error).
    """
    logger.info(f"Registration attempt for username: {user_data.username}")
    
    if not user_data.username or not user_data.password:
        logger.warning(f"Registration failed: Missing username or password for user: {user_data.username}")
        raise MissingRequiredFieldException("Username and password are required", 400)
    ...
    try:
        ...
        if result.scalar_one_or_none():
            ...
            raise UserAlreadyExistsException("Username already exists", 409)
    ...
    except (MissingRequiredFieldException, UserAlreadyExistsException):
        raise
    except Exception as e:
        logger.error(f"Registration error for user {user_data.username}: {str(e)}")
        await db.rollback()
        raise DatabaseTransactionException("Server error during registration", 500)
...
async def login(user_data: UserCreate, db: AsyncSession = Depends(get_db)) -> Token:
    ...
    Raises:
        MissingRequiredFieldException: If required fields are missing (400 Bad Request).
        InvalidCredentialsException: If the credentials are invalid (401 Unauthorized).
        DatabaseTransactionException: If there is a server error during login (500 Internal Server Error).
    """
    ...
    except (MissingRequiredFieldException, InvalidCredentialsException):
        raise
    except Exception as e:
        logger.error(f"Login error for user {user_data.username}: {str(e)}")
        raise DatabaseTransactionException("Server error during login", 500)
